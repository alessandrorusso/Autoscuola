...
$id=JRequest::getVar('id');
    ...
    <div style="font-size: 14px; font-weight: bold;">
      ...
      <a style="color: blue; text-decoration: none;" target="_blank" href="http://web-dorado.com/joomla-gallery-guide-step-2.html">Read More in User Manual</a>
    </div>
	 ...
	 function spider_set_href(a, number, type) {
      ...
      function bwg_add_image(files) {
		...
		if (!is_video) {
		  ...
		  a_crop.setAttribute('rel', "{handler: 'iframe', size: {x: 700, y: 600}}");
		  ...
		  a_rotate.setAttribute('rel', "{handler: 'iframe', size: {x: 700, y: 600}}");
          ...
          a_recover.setAttribute('onclick', 'if (confirm("Do you want to reset the image?")) { spider_set_input_value("ajax_task", "recover"); spider_set_input_value("image_current_id", "' + j + '"); spider_ajax_save("adminForm");} return false;');
          ...
          input_alt.setAttribute('size', "24");
		   ...
		   if (is_video) {
			input_alt.setAttribute('value', files[i]['filename'].substr(0,files[i]['filename'].indexOf('.')));
          }
          else {
          ...
          a_publish.setAttribute('onclick', "spider_set_input_value('ajax_task', 'image_unpublish');spider_set_input_value('image_current_id', '" + j + "');spider_ajax_save('adminForm');");
          ...
          a_delete.setAttribute('onclick', "spider_set_input_value('ajax_task', 'image_delete');spider_set_input_value('image_current_id', '" + j + "');spider_ajax_save('adminForm');");
...
width: 150px;
      ...
      <table style="clear:both;">
        <tbody>
          ...
          <tr>
            <td colspan=2>
              <?php
              echo image_display($id, $rows_data,$page_nav,$this,$this->get_option_rows_data);
              ?>
            </td>
          </tr>
        </tbody>
 ...
 function image_display($id,$rows_data,$page_nav,$this,$option_row) {
...
<a onclick="return bwg_check_checkboxes();" href="index.php?option=com_gallery_wd&view=addTags" rel="{handler: 'iframe', size: {x: 800, y: 500}}" class="button-primary modal">Add tag</a>
      ...
      <div class="tablenav top">
        <?php
        WDWLibrary::ajax_search('Filename', $search_value, 'adminForm');
        WDWLibrary::ajax_html_page_nav($page_nav['total'], $page_nav['limit'], 'adminForm');
        ?>
      </div>
            ...
            foreach ($rows_data as $row_data) {
              ...
              <tr id="tr_<?php echo $row_data->id; ?>" <?php echo $alternate; ?>>
                ...
                <td class="table_big_col"><a onclick="spider_set_input_value('ajax_task', 'image_<?php echo $published; ?>');
                                                      spider_set_input_value('image_current_id', '<?php echo $row_data->id; ?>');
                                                      spider_ajax_save('adminForm');"><img src="<?php echo  '../components/com_gallery_wd/images/' . $published_image . '.png'; ?>"></img></a>
													  </td>
                <td class="table_big_col"><a onclick="spider_set_input_value('ajax_task', 'image_delete');
                                                      spider_set_input_value('image_current_id', '<?php echo $row_data->id; ?>');
                                                      spider_ajax_save('adminForm');">Delete</a></td>
              </tr>
              <?php
  ...
  function get_user_name($user_id)
{
   ...
   function get_tag_rows_data($image_id) {
       $db =JFactory::getDBO();
    $query="SELECT table1.id as term_id,table1.name,table1.slug  FROM #__bwg_tags AS table1 INNER JOIN #__bwg_image_tag AS table2 ON table1.id=table2.tag_id WHERE table2.image_id=".$db->escape($image_id)." ORDER BY table2.tag_id";
	$db->setQuery($query);
