...
if ($saveOrder)
{
	...
	$saveOrderingUrl = 'index.php?option=com_gallery_wd&task=galleries.saveOrderAjax&tmpl=component';
...
Joomla.tableOrdering= function ( order, dir, task )  {
    ...
    <form class="wrap" id="adminForm" name="adminForm" method="post" action="index.php?option=com_gallery_wd">
...
<th width="1%" class="nowrap center hidden-phone">
				...
				<th nowrap="nowrap" width="50">Thumbnail</th>
        ...
        <tbody id="tbody_arr" class='ui-sortable'>
		  ...
		  $i=0;
            foreach ($rows_data as $row_data) {
			  ...
			  $edit_link='index.php?option=com_gallery_wd&view=galleries&task=edit&cid[]='.$row_data->id;
			  ...
			  jQuery(this).parent().parent().children().find('input')[1].checked='checked'
			   spider_set_input_value('task', 'galleries.delete');
...
if(jQuery('#filter_order_Dir').val()=='asc')
i=0;
else
i=jQuery('#tbody_arr').find('tr').length-1;
...
i--
					...
					/*var sortableList = new $.JSortableList('#articleList tbody','adminForm','desc' , 'index.php?option=com_gallery_wd&task=galleries.saveOrderAjax&tmpl=component','','');*/
...
function published( &$row, $i, $task, $imgY = 'tick.png', $imgX = 'publish_x.png', $prefix='' ){
        ...
        $task     = $row->published ? 'galleries.unpublish' : 'galleries.publish';
