...
exit; */  
   $mainframe->redirect( 'index.php?option=com_gallery_wd&task=edit&cid[]='.$id,'','message' );
    ...
    if ($gallery_ids_col) {
      foreach ($gallery_ids_col as $gallery_id) {
		...
		$query="SELECT `order` FROM #__bwg_gallery WHERE `id`=".$db->escape($gallery_id);
		$db->setQuery($query);
	  ...
	  $query="UPDATE #__bwg_gallery SET `order`=".$db->escape($i)." WHERE id=".$db->escape($key);
     $db->setQuery($query);
	 $db->query();
        $i++;
      }
  ...
  public function save_db() {
    ...
    {
      if ($id != 0) {
        $query="SELECT random_preview_image FROM #__bwg_gallery WHERE id=".$db->escape($id);
		$db->setQuery($query);
		
		$random_preview_image =$db->loadResult();
        
		if ($random_preview_image == '') {
		 $query="SELECT thumb_url FROM #__bwg_image WHERE gallery_id=".$db->escape($id)." ORDER BY `order`";
		$db->setQuery($query);
        $random_preview_image =$db->loadResult();
        }
      }
  ...
  public function bwg_get_unique_name($name, $id) {
    ...
    if ($id != 0) {
      $query = "SELECT name FROM #__bwg_gallery WHERE name = '".$db->escape($name)."' AND id != ".$db->escape($id);
    }
    else {
  ...
  public function bwg_get_unique_slug($slug, $id) {
    ...
    if ($id != 0) {
      $query = "SELECT slug FROM #__bwg_gallery WHERE slug = '". $db->escape($slug)."' AND id != ".$db->escape($id);
    }
    else {
	...
	$id=$image_id;
	 ...
	 $query="SELECT published FROM #__bwg_image WHERE id=".$db->escape($id);
	 $db->setQuery($query);
  ...
  if (JRequest::getVar('check_' . $image_id)!="") {
		...
		$query="SELECT tag_id FROM #__bwg_image_tag WHERE image_id=".$db->escape($row->id)." AND gallery_id=".$db->escape($gal_id);
		$db->setQuery($query);
		$deleted_tag=$db->loadResult();

		$query="DELETE FROM #__bwg_image_tag WHERE image_id=".$db->escape($row->id)." AND gallery_id=".$db->escape($gal_id);
 ...
 if($deleted_tag)
 {
        $query="SELECT COUNT(image_id) FROM #__bwg_image_tag WHERE tag_id=".$db->escape($deleted_tag);
	    $db->setQuery($query);
	 $image_count=$db->loadResult();
			  
	 $query="UPDATE #__bwg_tags SET count=".$image_count." WHERE id=".$db->escape($deleted_tag);
	$db->setQuery($query);
	 $db->query();
          ...
          foreach ($tag_id_array as $tag_id) {
            if ($tag_id) {
			  ...
			  $query="SELECT COUNT(image_id) FROM #__bwg_image_tag WHERE tag_id=".$db->escape($tag_id);
			  $db->setQuery($query);
			  $image_count=$db->loadResult();
			  
			  $query="UPDATE #__bwg_tags SET count=".$db->escape($image_count)." WHERE id=".$db->escape($tag_id);
			  $db->setQuery($query);
			  $db->query();
   ...
   public function save_order_images($gallery_id) {
    $db =JFactory::getDBO();
	$query="SELECT id FROM #__bwg_image WHERE `gallery_id`=".$db->escape($gallery_id);
	$db->setQuery($query);
	$p='';
    ...
    if ($imageids_col) {
      foreach ($imageids_col as $imageid) {
		...
		$query="SELECT `order` FROM #__bwg_image WHERE `id`=".$db->escape($imageid);
		$db->setQuery($query);
      ...
      $i = 1;
      foreach ($order_values as $key => $order_value) {
	  $query="UPDATE #__bwg_image SET `order`=".$db->escape($i)." WHERE `id`=".$db->escape($key);
$db->setQuery($query);
$db->query();
        $i++;
      }
...
if($id=='' or $id==0)
{
  ...
  public function image_delete() {
	...
	$query='DELETE FROM #__bwg_image WHERE id="'.$db->escape($id).'"';
	$db->setQuery($query);
	$db->query();
	
	$query='DELETE FROM #__bwg_image_comment WHERE image_id="'.$db->escape($id).'"';
	$db->setQuery($query);
	$db->query();
	
	$query="SELECT tag_id FROM #__bwg_image_tag WHERE image_id=".$db->escape($id);
	$db->setQuery($query);
	$tag_ids = $db->loadColumn();
	
	$query='DELETE FROM #__bwg_image_tag WHERE image_id="'.$db->escape($id).'"';
	$db->setQuery($query);
	$db->query();
	...
	if ($tag_ids) {
	  ...
	  $query="UPDATE #__bwg_tags SET count='".$count."' WHERE id=".$db->escape($tag_id);
		$db->setQuery($query);
		$db->query();	  
      }
    }
 ...
 $db =JFactory::getDBO();
	...
	$query='DELETE FROM #__bwg_image WHERE id="'.$db->escape($image_id).'"';
	$db->setQuery($query);
	
	$db->query();
	
	$query='DELETE FROM #__bwg_image_comment WHERE image_id="'.$db->escape($image_id).'"';
	$db->setQuery($query);
	$db->query();
	
	$query="SELECT tag_id FROM #__bwg_image_tag WHERE image_id=".$db->escape($image_id);
	$db->setQuery($query);
	$tag_ids = $db->loadColumn();
	
	$query='DELETE FROM #__bwg_image_tag WHERE image_id="'.$db->escape($image_id).'"';
	$db->setQuery($query);
	$db->query();
	...
	if ($tag_ids) {
	  ...
	  $query="UPDATE #__bwg_tags SET count='".$db->escape($count)."' WHERE id=".$db->escape($tag_id);
		$db->setQuery($query);
		$db->query();	  
      }
    }
    ...
    public function image_publish() {
	    $db =JFactory::getDBO();
    $id =JRequest::getVar('image_current_id') ;
    $query="UPDATE #__bwg_image SET published=1 WHERE id=".$db->escape($id);
    ...
    if (JRequest::getVar('check_all_items')!='') {
	 $query="UPDATE #__bwg_image SET published=1 WHERE gallery_id=".$db->escape($gallery_id);
    $db->setQuery($query);
	$db->Query();
    }
    else {
    ...
    $image_ids_col = $db->loadObjectList();
      ...
      foreach ($image_ids_col as $image_id) {
        if (JRequest::getVar('check_' . $image_id->id)!="") {
		      $query="UPDATE #__bwg_image SET published=1 WHERE id=".$db->escape($image_id->id);
  ...
  public function image_unpublish_all() {
    ...
    $gallery_id = JRequest::getVar('current_id',0) ;
    if (JRequest::getVar('check_all_items')!='') {
	$query="UPDATE #__bwg_image SET published=0 WHERE gallery_id=".$db->escape($gallery_id);
    $db->setQuery($query);
	$db->Query();
    }
    else {
    ...
    $image_ids_col = $db->loadObjectList();
      ...
      foreach ($image_ids_col as $image_id) {
        if (JRequest::getVar('check_' . $image_id->id)!="") {
		      $query="UPDATE #__bwg_image SET published=0 WHERE id=".$db->escape($image_id->id);
    ...
    public function image_unpublish() {
	    $db =JFactory::getDBO();
		$id =JRequest::getVar('image_current_id') ;
		$query="UPDATE #__bwg_image SET published='0' WHERE id='".$db->escape($id)."'";
  ...
  $db->setQuery($query);
	...
	$gallery_id=0;
	    ...
	    $query="SELECT * FROM #__bwg_image WHERE gallery_id=".$db->escape($gallery_id);
  $db->setQuery($query);
...
$db=JFactory::getDBO();
	...
	$query='SELECT * FROM #__bwg_image WHERE gallery_id='.$gallery_id;
	$db->setQuery($query);
	$images =$db->loadObjectList();
    foreach ($images as $image) {
      if (JRequest::getVar('check_'. $image->id)!='' ||JRequest::getVar('check_all_items')!='') {
