   ...
   $mainframe->redirect( 'index.php?option=com_gallery_wd&view=albums&task=edit&cid[]='.$id,'','message' );
    ...
    if ($album_ids_col) {
      foreach ($album_ids_col as $album_id) {
		...
		$query="SELECT `order` FROM #__bwg_album WHERE `id`=".$db->escape($album_id);
		$db->setQuery($query);
	  ...
	  $query="UPDATE #__bwg_album SET `order`=".$db->escape($i)." WHERE id=".$db->escape($key);
     $db->setQuery($query);
	 $db->query();
        $i++;
      }
...
if($row->order==0)
    ...
    {
      if ($id != 0) {
        $query="SELECT random_preview_image FROM #__bwg_album WHERE id=".$db->escape($id);
		$db->setQuery($query);
...
$this->save_alb_gal($row->id, $albums_galleries);
    ...
    $query = 'DELETE FROM #__bwg_album_gallery WHERE album_id='.$db->escape($id);
    $db->setQuery($query);
	$db->Query();
 ...
 $row->id='';
  ...
  public function bwg_get_unique_name($name, $id) {
    ...
    if ($id != 0) {
      $query = "SELECT name FROM #__bwg_album WHERE name = '".$name."' AND id != ".$db->escape($id);
    }
    else {
  ...
  public function bwg_get_unique_slug($slug, $id) {
    ...
    if ($id != 0) {
      $query = "SELECT slug FROM #__bwg_album WHERE slug = '". $db->escape($slug)."' AND id != ".$db->escape($id);
    }
    else {
