  ...
  public function get_row_data($id) {
   ...
   //!current_user_can('manage_options') && 
    if ($id != 0) {
	  ...
	  $query = 'SELECT * FROM #__bwg_album ' . $where . ' AND id='.$id;
   $db->setQuery($query);
	$row=$db->loadObject();
   ...
   //!current_user_can('manage_options') &&
	...
	$query="SELECT t1.id, t2.name, t2.slug, t1.is_album, t1.alb_gal_id, t1.order FROM #__bwg_album_gallery as t1 INNER JOIN #__bwg_album as t2 on t1.alb_gal_id = t2.id where t1.is_album='1'" . $where . " AND t1.album_id='" . $db->escape($album_id) . "' union SELECT t1.id, t2.name, t2.slug, t1.is_album, t1.alb_gal_id, t1.order FROM #__bwg_album_gallery as t1 INNER JOIN #__bwg_gallery as t2 on t1.alb_gal_id = t2.id where t1.is_album='0'" . $where . " AND t1.album_id='" . $db->escape($album_id) . "' ORDER BY `order`";
