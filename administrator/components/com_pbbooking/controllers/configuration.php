...
class PbbookingsControllerconfiguration extends JControllerLegacy {
    ...
    function save() {
        ...
        foreach ($fields as $field => $type) {
            if ($type == 'integer' && !(isset($config[$field])))
                $config[$field] = 0;
        }
        ...
        if ($id != 0) {
            ...
            $db->insertObject('#__pbbooking_config', new JObject($config));
        }

        $this->setRedirect('index.php?option=com_pbbooking');
    }
