	...
	*/
	...
	function edit()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == "POST") {
			...
			$dtstart = date_create(JRequest::getVar('date'),new DateTimeZone(PBBOOKING_TIMEZONE));
	...
	*/
	function create()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == "POST") {
			...
			if ($event_id) {
				...
				$this->setRedirect('index.php?option=com_pbbooking&controller=manage&date='.JRequest::getVar('date'),JText::_('COM_PBBOOKING_CREATE_ERROR').' '.$db->getErrorMsg());
			}
		}	
	}
	...
	function save_block_days()
	{
		...
		if ($_POST['block-from-date'] && $_POST['block-end-date']) {
			...
			if ($make_recurring == 1) $block->setProperties(array('r_int'=>$r_int,'r_freq'=>$r_freq,'r_end'=>date_create($r_end,new DateTimeZone(PBBOOKING_TIMEZONE))->format(DATE_ATOM)));
