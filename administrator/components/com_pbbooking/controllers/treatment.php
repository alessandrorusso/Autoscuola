    ...
    function save() {
        ...
        if ($id != 0) {
            $sql = sprintf("update #__pbbooking_treatments set name = '%s',duration=%s,price=%s, calendar='%s',
							require_payment = '%s' where id = %s", $name, $duration, $price, $db->escape(implode(',', $calendar)), $db->escape($require_payment), $id);
        } else {
            $sql = sprintf("insert into #__pbbooking_treatments (name,duration,price,calendar,require_payment) values ('%s',%s,%s,'%s',%s)", $name, $duration, $price, $db->escape(implode(',', $calendar)), $db->escape($require_payment));
        }
        $db->setQuery($sql);
        $db->query();
