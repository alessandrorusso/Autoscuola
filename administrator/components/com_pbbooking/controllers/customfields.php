    ...
    function save() {
        ...
        if ($id != 0) {
            $sql = sprintf("update #__pbbooking_customfields set 
					...
					fieldtype='%s',
					varname='%s',
					size=%s, is_email = %s, is_first_name = %s, is_last_name = %s, is_required = %s, `values` = '%s' where id = %s", $fieldname, $fieldtype, $varname, $size, $db->escape($is_email), $db->escape($is_first_name), $db->escape($is_last_name), $db->escape($is_required), $db->escape($values), $id);
        } else {
            $sql = sprintf("insert into #__pbbooking_customfields 
					...
					('%s','%s','%s',%s,%s,%s,%s,%s,'%s')", $fieldname, $fieldtype, $varname, $size, $db->escape($is_email), $db->escape($is_first_name), $db->escape($is_last_name), $db->escape($is_required), $db->escape($values));
        }
        $db->setQuery($sql);
        $db->query();
        //echo $db->getErrorMsg();
