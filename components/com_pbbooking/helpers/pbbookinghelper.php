...
class Pbbookinghelper
{
   ...
   /** 
    * get calendar users - returns all users for the given calendar
    * @param calendar 
    * @return bool
    * @since 2.0.6
    * @access public
    */
    ...
    public static function validate_event($treatment_id, $dtstart, $user, $calId){        
        $db = JFactory::getDbo();
        $config =JFactory::getConfig();
        date_default_timezone_set($config->get('offset'));
	
        $db->setQuery('select * from #__pbbooking_treatments where id = '.$db->escape($treatment_id));
        $treatment = $db->loadObject();
        if(!$treatment->id > 0){
            throw new Exception('Problemi nel salvataggio della prenotazione, si prega di riprovare.');
        }
        
        $now = date_create("now",new DateTimeZone(PBBOOKING_TIMEZONE));        
        if($dtstart < $now){            
            throw new Exception('Non è possibile selezionare una data passata!');
        }
        $now->modify('+ 1 Days');
        if($dtstart <= $now){
            throw new Exception('Occorre prenotare con più di 24 ore di anticipo!');
        }
        $now->modify('+ 14 Days');
        
        if($dtstart > $now){
            throw new Exception('E\' possibile prenotare con un massimo di 15 giorni di anticipo!');
        }
        ...
        if ($calId != 0) {
            ...
            if($cal){
                ...
                if($user instanceof JUser){
                    if(!$registrationGroupUsers[$user->id]){
                        throw new Exception("L\'Utente selezionato non è abilitato al Calendario per il quale si vuole effettuare la prenotazione!");
                    }
                }
                else{
                    throw new Exception("Selezionare un Utente valido!");
                }                    
            }
        }
        else{
            return "Selezionare un Calendario valido!";
        }
