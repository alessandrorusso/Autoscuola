...
$query="SELECT * FROM #__bwg_theme WHERE default_theme=1"	;
  ...
  public function get_comment_rows_data($image_id) {
    $db =JFactory::getDBO();
	$query="SELECT * FROM #__bwg_image_comment WHERE image_id=".(int)$db->escape($image_id)." AND published=1 ORDER BY `id` DESC";
	$db->setQuery($query);
  ...
  public function get_image_rows_data($gallery_id, $sort_by,$order_by="asc") {
	...
	$query='SELECT t1.*,t2.rate FROM #__bwg_image as t1 LEFT JOIN (SELECT rate, image_id FROM #__bwg_image_rate WHERE ip="'.$_SERVER['REMOTE_ADDR'].'") as t2 ON t1.id=t2.image_id WHERE t1.published=1 AND t1.gallery_id='.$db->escape($gallery_id).' ORDER BY ' . $sort_by . ' ' . $order_by;
