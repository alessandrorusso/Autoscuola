...
$document->addScript(JURI::root() . 'components/com_gallery_wd/js/jquery.raty.js');		
...
if (isset($params['type'])) {
			...
			$this->get_image_rows_data=$this->get_image_rows_data($params['gallery_id'],  $images_per_page, $params['sort_by'], $bwg,$type);
			...
			$this->get_image_rows_data=$this->get_image_rows_data($album_gallery_id, $items_per_page, $params['sort_by'], $bwg);
...
return $url;
  ...
  public function get_image_rows_data($id, $images_per_page, $sort_by, $bwg, $type='') {
	...
	if($type == 'tag') {
      $query = 'SELECT image.* FROM #__bwg_image as image INNER JOIN #__bwg_image_tag as tag ON image.id=tag.image_id WHERE image.published=1 '. $where .' AND tag.tag_id='.$db->escape($id).' ORDER BY ' . $sort_by . ' ASC';      
    }
	else
	{
	$query='SELECT * FROM #__bwg_image WHERE published=1 '. $where .' AND gallery_id='.$db->escape($id).' ORDER BY ' . $sort_by . ' ASC ' . $limit_str;
	}
	$db->setQuery($query);
    $row = $db->loadObjectList();
    return $row;
