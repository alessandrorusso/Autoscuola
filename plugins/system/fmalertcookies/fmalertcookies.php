...
class plgSystemFmAlertCookies extends JPlugin
{
	...
	function onBeforeCompileHead(){
		...
		if ($app->isSite()) {
			...
			if ($position = $this->params->get('ajouter_jquery')) {
	        	if (JVERSION < 3) {
					$document->addScript(JURI::root().'plugins/system/fmalertcookies/assets/js/jquery-1.11.1.min.js');
				} else {
			...
			if(!file_exists(dirname(__FILE__).'/assets/css/custom.css')){
				fopen(dirname(__FILE__).'/assets/css/custom.css',"w");
			}
			...
			if($this->params->get('type_affichage') == 0) {
				$document->addScript(JURI::root().'plugins/system/fmalertcookies/assets/js/jquery.magnific-popup.min.js');
	...
	public function onContentPrepareForm($form, $data)
	{
		...
		foreach ($languages as $tag => $language) {
			...
			$language->lang_code = str_replace('-', '_', $language->lang_code);
				...
				<form>
					<fields name="params">
						<fieldset name="'.$langue_name.'" label="'.JText::_('PLG_SYSTEM_FMALERTCOOKIES_SAISIE_LANGUE').' '.$langue_name.'">
						   ...
						   <field name="texte_alert_cookies_'.$language->lang_code.'" type="editor" height="250"
	...
	function onAfterRender()
    {
		...
		if ($app->getCfg('offline') == 0) {
	        ...
	        if (!$app->isSite() || $lang_activate == 0 || (strtolower($_SERVER['REQUEST_URI']) == strtolower(JRoute::_("index.php?Itemid=".$link_readmore_menu)) && !$affichage_msg_page_cookie)) {
	            return;
	        }
			...
			elseif($type_affichage == 0) {
				...
				$popup .='if (c.indexOf(name) == 0){ jQuery.magnificPopup.close();}';
				$popup .='}';
				$popup .="</script>";
