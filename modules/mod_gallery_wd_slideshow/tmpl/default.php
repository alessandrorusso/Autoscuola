...
defined('_JEXEC') or die('Restricted access');
 ...
 require_once JPATH_SITE.'/modules/mod_gallery_wd_slideshow/helper.php';
...
$options_row = Modgallery_wdSlideShowHelper::get_options_row_data_slide();
    ...
    $filmstrip_direction = 'horizontal';
      ...
      $theme_id = (isset($slide_params['theme_id']) ? htmlspecialchars($slide_params['theme_id']) : 0);
      $gallery_id = (isset($slide_params['gallery_id']) ? htmlspecialchars($slide_params['gallery_id']) : 0);
      $sort_by = 'order';
      $slideshow_effect = "fade";
      $enable_slideshow_autoplay = $options_row->slideshow_enable_autoplay;
      $enable_slideshow_shuffle = (isset($slide_params['slideshow_enable_shuffle']) ? htmlspecialchars($slide_params['slideshow_enable_shuffle']) : 0);
      ...
      $image_width = (isset($slide_params['width']) ? htmlspecialchars($slide_params['width']) : '800');
      $image_height = (isset($slide_params['height']) ? htmlspecialchars($slide_params['height']) : '600');
      $slideshow_interval = (isset($slide_params['interval']) ? htmlspecialchars($slide_params['interval']) : 5);
 ...
 $theme_row = Modgallery_wdSlideShowHelper::get_theme_row_data_slide($theme_id);
    if (!$theme_row) {
      echo WDWLibrary::message(JText::_('THERE_IS_NO_THEME'), 'error');
      return;
    }
    $gallery_row = Modgallery_wdSlideShowHelper::get_gallery_row_data_slide($slide_params['gallery_id']);
    if (!$gallery_row) {
      echo WDWLibrary::message(JText::_('THERE_IS_NO_GALLERY'), 'error');
      return;
    }
    $image_rows = Modgallery_wdSlideShowHelper::get_image_rows_data_slide($slide_params['gallery_id'],  '', $sort_by, $bwg);
    if (!$image_rows) {
