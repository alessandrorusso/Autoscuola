...
LEFT JOIN #__bwg_gallery ON #__bwg_album_gallery.alb_gal_id=#__bwg_gallery.id
	 ...
	 WHERE album_id='.$db->escape($id).' AND published=1 ORDER BY ' . ($sort_by == "RAND()" ? '' : '`') . $sort_by . ($sort_by == "RAND()" ? '' : '`') . ' ASC ' . $limit_str;    $db->setQuery($query);
	$row = $db->loadObjectList();
    return $row;
  }
  ...
  function bgw_url_encode_gal_alb($url)
{
...
function bgw_url_decode_gal_alb($url)
{
$url=str_replace('bwg_dots',':',$url);
$url=str_replace('bwg_slash','/',$url);
$url=str_replace('bwg_equal','=',$url);
